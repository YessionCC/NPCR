...
class Generatic_Model(torch.nn.Module):
    ...
    def __init__(self, tar_width, tar_height, layer_num, depth_thr, vertex_list, feature_dim, dataset=None, use_rgb = False ):
        super(Generatic_Model, self).__init__()
        self.dataset = dataset
        self.pcpr_parameters = PCPRParameters(vertex_list, feature_dim)
        self.render = PCPRender(feature_dim,tar_width,tar_height, layer_num, depth_thr, dataset = dataset)
        input_channels = 0
    ...
    def forward(self, point_indexes, in_points, K, T,
        ...
        res,depth,inds,pw,_ = self.render(point_features, default_features,
                             in_points,
                             K, T,
                             near_far_max_splatting_size, num_points, inds)

        
        return res,depth,inds,pw
